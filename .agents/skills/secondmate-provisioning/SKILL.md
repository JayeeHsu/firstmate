...
You resolve each project's origin yourself - from the captain, the project registry, a clone that exists elsewhere, the native `gh` CLI for GitHub, `glab` or the existing GitLab integration for GitLab, or an explicit paste - and name it as `<project>=<origin-url>`; the seed validates and transports what you supply.
