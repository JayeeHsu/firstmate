...
printf '%s\n' "$*" >> "$FM_TEST_GH_LOG"
case "${1:-} ${2:-}" in
  "pr merge") printf '%s\n' "$*" >> "$FM_TEST_GH_MERGE_LOG"; exit "${FM_TEST_GH_MERGE_RC:-0}" ;;
  "api graphql")
    printf '%s\n' \
      'state=MERGED' \
      'merged=true' \
...
run_check_entry() {
  ...
  FM_ROOT_OVERRIDE="$dir/root" FM_HOME="$dir/home" \
    ...
    FM_TEST_GH_MERGE_LOG="$dir/gh-merge.log" FM_TEST_GLAB_LOG="$dir/glab.log" \
...
test_invalid_entrypoints_have_zero_side_effects() {
  ...
  [ ! -s "$dir/gh-merge.log" ] || fail "invalid direct or merge data called native gh"
...
EOF
  ...
  [ ! -s "$dir/gh-merge.log" ] || fail "merge wrapper reached the GitHub CLI for a GitLab URL"
  grep -qF "mr view 7 -R https://gitlab.example/group/subgroup/project" "$dir/glab.log" \
