...
exit 0
SH
  # Default native gh mock: no PR is associated with the branch, and viewing
  # any PR number fails. This keeps the landed-work check hermetic and represents
  # the common "no GitHub PR" baseline. Tests that need a merged PR or a lookup
  # error override this file with the helpers below.
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
case "${1:-} ${2:-}" in
  "pr list") exit 0 ;;
...
# Override native gh lookups to report PR 7 as merged with the supplied head.
...
# Override native gh so every call fails, simulating an API/network error.
add_gh_error() {
  local case_dir=$1
  cat > "$case_dir/fakebin/gh" <<'SH'
#!/usr/bin/env bash
...
test_no_mistakes_truly_unpushed_refuses() {
  ...
  write_meta "$case_dir" no-mistakes ship
  # Real content that is not pushed, has no PR (default native gh mock), and never
...
test_content_in_default_fallback_allows() {
  ...
  write_meta "$case_dir" no-mistakes ship
  # No pr= recorded and the default native gh mock reports no PR, so the merged-PR path
