...
The universal toolchain is node, git, the native gh CLI with GitHub auth via `gh auth login`, no-mistakes v1.46.0 or newer, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
In that list, no-mistakes runs the validation pipeline, the native `gh` CLI covers GitHub operations, and lavish-axi covers rich-review operations, while tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch.
Browser automation and UI acceptance use the separately installed Ego Lite / `ego-browser`; GitLab operations use `glab` or the existing GitLab integration as appropriate.
