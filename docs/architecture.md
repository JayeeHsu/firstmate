...
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh pr merge <n> --repo github.com/<owner>/<repo>`, defaults to `--squash`, and preserves explicit merge-method flags; subsequent outcome and branch-rule reads use `gh api --hostname github.com` so ambient CLI host configuration cannot redirect any operation.
...
Every GitHub refusal states what it could not observe as plainly as what it did: an unreadable outcome stops verification, while a readable open outcome distinguishes an unreadable branch-rule response and an unrecognised queue method from a base branch with no queue rule.
